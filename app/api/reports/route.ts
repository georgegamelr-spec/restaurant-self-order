  ...
  try {
    ...
    activeOrders.forEach(o => {
      ;(o.items||[]).forEach((it:{name?:string;name_ar:string;emoji:string;price:number;qty:number}) => {
        const k = it.name_ar
        if (!itemMap[k]) itemMap[k]={name:it.name||it.name_ar,name_ar:it.name_ar,emoji:it.emoji||'🍽️',qty:0,revenue:0}
        itemMap[k].qty+=it.qty; itemMap[k].revenue+=it.price*it.qty
      })
    })
    ...
    cancelledOrders.forEach(o=>{const r=o.cancel_reason||'غير محدد';reasonMap[r]=(reasonMap[r]||0)+1})
    ...
    const hourlyBreakdown = Object.entries(hourMap).map(([hour,v])=>({hour,...v})).sort((a,b)=>a.hour.localeCompare(b.hour))
    ...
    activeOrders.forEach(o=>{
      ...
      dayMap[day].count++; dayMap[day].total+=o.total||0
    })
    const dailyBreakdown = Object.entries(dayMap).map(([date,v])=>({date,...v}))
    ...
    activeOrders.forEach(o=>{
      ...
      tableMap[t].orders++; tableMap[t].total+=o.total||0
    })
    const tableBreakdown = Object.entries(tableMap).map(([table,v])=>({table,...v})).sort((a,b)=>b.total-a.total)

    return NextResponse.json({
      range, from: from.toISOString(), to: to.toISOString(),
      summary:{totalOrders:activeOrders.length,cancelledOrders:cancelledOrders.length,
        totalRevenue:Math.round(totalRevenue*100)/100,netRevenue:Math.round(netRevenue*100)/100,
        totalTax:Math.round(totalTax*100)/100,avgOrderValue:Math.round(avgOrderValue*100)/100,totalItemsSold},
      topItems,bottomItems,cancelledItems,cancelReasons,
      hourlyBreakdown,peakHour,dailyBreakdown,tableBreakdown,
    })
  } catch(err:unknown){
