...
// Direct PostgreSQL connection using Supabase DB URL
const pool = new Pool({
...
const STEPS = [
  ...
  {
    name: 'function_update_updated_at',
  ...
  {
    name: 'trigger_suppliers',
    sql: `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_suppliers_updated')
      THEN CREATE TRIGGER trg_suppliers_updated BEFORE UPDATE ON suppliers
      FOR EACH ROW EXECUTE FUNCTION update_updated_at(); END IF; END $$`
  },
  {
    name: 'trigger_ingredients',
    sql: `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_ingredients_updated')
      THEN CREATE TRIGGER trg_ingredients_updated BEFORE UPDATE ON ingredients
      FOR EACH ROW EXECUTE FUNCTION update_updated_at(); END IF; END $$`
  },
  {
    name: 'trigger_invoices',
    sql: `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_invoices_updated')
      THEN CREATE TRIGGER trg_invoices_updated BEFORE UPDATE ON supplier_invoices
      FOR EACH ROW EXECUTE FUNCTION update_updated_at(); END IF; END $$`
  },
  {
    name: 'function_stock_movement',
    sql: `CREATE OR REPLACE FUNCTION apply_stock_movement() RETURNS TRIGGER AS $fn$
      BEGIN
  ...
  {
    name: 'trigger_stock_movement',
    sql: `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_stock_movement')
      THEN CREATE TRIGGER trg_stock_movement AFTER INSERT ON stock_movements
      FOR EACH ROW EXECUTE FUNCTION apply_stock_movement(); END IF; END $$`
  },
]
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    message: failed.length === 0 ? '✅ Migration completed!' : `⚠️ ${failed.length} steps failed`
  })
}
